...
main <- function()
{
	...
	bmi_snps <- read.table("~/repo/mr_frailty/data-raw/locke_clumped_0.001.txt", he=T)
...
bmi_survival <- function(age, bmi)
{
	# hr of 1.06 per sd increase in bmi
	survival <- (1 - gompertz_makeham_cdf(age)) ^ (bmi * log(1.16)/sd(bmi, na.rm=TRUE))
	return(survival)
}
